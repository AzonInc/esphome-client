...
async function handleList(options, commandOptions) {
    if (!options.host) {
        console.error("Error: --host option is required.");
        process.exit(1);
    }
...
async function handleMonitor(options, commandOptions) {
    if (!options.host) {
        console.error("Error: --host option is required.");
        process.exit(1);
    }
...
async function handleControl(options, commandArgs, commandOptions) {
    if (!options.host) {
        console.error("Error: --host option is required.");
        process.exit(1);
    }
        ...
        switch (entityType) {
            ...
            case "light": {
                ...
                if (commandOptions.rgb && typeof commandOptions.rgb === "string") {
                    const parts = commandOptions.rgb.split(",").map((v) => parseInt(v.trim()));
            ...
            case "lock": {
                ...
                if (cmd === "lock" || cmd === "unlock" || cmd === "open") {
                    ...
                    client.sendLockCommand(entity.id, cmd);
                }
                else {
                        ...
                        switch (entityType) {
                            ...
                            case "climate": {
                                ...
                                if (mode && ["off", "heat", "cool", "heat_cool", "auto", "dry", "fan_only"].includes(mode)) {
                                    ...
                                    client.sendClimateCommand(entity.id, { mode: climateMode });
